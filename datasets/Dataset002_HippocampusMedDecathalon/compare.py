...
Compares Dataset002_HippocampusMedDecathalon configurations between:
...
class ConfigComparator:
    ...
    def compare_dataset_properties(self) -> None:
        ...
        # Spacing
        nnunet_spacing = plans_3d["spacing"]
        ...
        if "shapes_after_crop" in self.fingerprint:
            ...
            # Calculate statistics on spatial dimensions from nnUNet fingerprint
            # For 3D data, shapes are [depth, height, width] without channel dimension
            # For 2D data, shapes would be [1, height, width] with channel dimension
            # Dataset002 is 3D, so shapes are already spatial-only
            spatial_shapes = shapes_after_crop  # Use directly for 3D data
            ...
            try:
                    ...
                    for img_path in image_files:
                        try:
                            ...
                            if len(shape) == 4:  # Has channel dimension
                                ...
                                nnbenchmark_shapes.append(shape)
                        except Exception:
                ...
                nnbenchmark_min_shape = "nibabel not installed"
                nnbenchmark_max_shape = "nibabel not installed"
                nnbenchmark_num_images = "nibabel not installed"
    ...
    def compare_architecture(self) -> None:
        ...
        self.add_result("Spatial Dimensions", 3, nnbenchmark_dims)
        ...
        # Strides (critical: first level must be [1,1,1] for full resolution in 3D)
        ...
        if nnbenchmark_strides[0] == [1, 1, 1]:
            self.add_result(
        ...
        self.add_result(
            ...
            nnunet_norm,
            nnbenchmark_norm,
            note="InstanceNorm3d (nnUNet) == INSTANCE (MONAI)",
        )
    ...
    def compare_training(self) -> None:
        ...
        # Batch size
        nnunet_batch = plans_3d["batch_size"]
    ...
    def compare_loss(self) -> None:
        ...
        # Batch dice (3D full resolution typically uses batch_dice=False)
        plans_3d = self.plans["configurations"]["3d_fullres"]
        nnunet_batch_dice = plans_3d.get("batch_dice", False)
    ...
    def compare_normalization(self) -> None:
        ...
        nnunet_norm = plans_3d["normalization_schemes"][0]
        ...
        # Use mask for norm
        nnunet_mask = plans_3d["use_mask_for_norm"][0]
    ...
    def compare_augmentation(self) -> None:
        ...
        for aug in expected_augs:
            ...
            count_note = f" (count: {aug_counts[aug]})" if has_aug else ""
            self.add_result(
                f"Augmentation: {aug}",
                "Present",
                "Present" if has_aug else "Missing",
                note=count_note if count_note else "",
            )

        # Rotation probability and range (3D has range_x, range_y, range_z)
        rotation_transforms = [
        ...
        if rotation_transforms:
            ...
            # For 3D, check all rotation ranges
            nnunet_rot_range = 0.5236  # 30 degrees in radians
            for axis in ["x", "y", "z"]:
                nnbenchmark_rot_range = rot.get(f"range_{axis}", 0.0)
                self.add_result(
                    f"Rotation Range {axis.upper()} (radians)",
        ...
        # Flipping (3D has 3 flip transforms, one per axis)
        flip_count = aug_counts.get("RandFlipd", 0)
        self.add_result(
            "Flip Transforms Count",
            3,
            flip_count,
            note="3D requires one flip transform per axis",
        )
    ...
    def run_comparison(self) -> int:
        ...
        print("Dataset: Dataset002_HippocampusMedDecathalon")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare nnBenchmark and nnUNet configurations for Dataset002_HippocampusMedDecathalon"
    )
    parser.add_argument(
        "--fingerprint",
        type=Path,
        default=Path(
            "/home/localssk23/CAI4Soumya/SegData/nnUNet_preprocessed/Dataset002_HippocampusMedDecathalon/dataset_fingerprint.json"
        ),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "/home/localssk23/CAI4Soumya/SegData/nnUNet_results/Dataset002_HippocampusMedDecathalon/nnUNetTrainer_50epochs__nnUNetPlans__3d_fullres/plans.json"
        ),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "/home/localssk23/CAI4Soumya/SegData/nnUNet_results/Dataset002_HippocampusMedDecathalon/fold_0/fold_0.yaml"
        ),
